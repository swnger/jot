...
import {
  authFilePath,
  commenterCookieMaxAgeSeconds,
  commenterIdCookieName,
  commenterNameCookieName,
  dataDir,
  notesDir,
  ownerCookieMaxAgeSeconds,
...
app.get("/", requireOwnerPage, (_req, res, next) => {
  next();
});

app.get("/notes/:id", requireOwnerPage, (req, res, next) => {
...
async function loadSvelteKitHandler() {
  try {
    // SvelteKit's adapter-node output is generated by `vite build` before this server is compiled.
    // @ts-expect-error generated file is outside the TypeScript project graph
    const module = await import("../build/handler.js") as { handler: RequestHandler };
...
const svelteKitHandler = await loadSvelteKitHandler();
if (svelteKitHandler) {
